...
| 4 | 검색 결과 및 기온에 따른 3단계 조건부 렌더링 | `v-if`, `v-else-if`, `v-else` |
...
- 도시 이름 부분 검색 및 선택한 입력 모드에 따른 필터링
  - 예: `부` 입력 시 `부산, 부천`을 모두 표시
- 기온에 따라 더움(25°C 이상), 보통(20~24°C), 선선함(20°C 미만) 라벨 분기
- 실시간 `:value + @input`과 지연 `v-model.trim.lazy` 동작 비교
- 두 입력 모드 모두 `update-query` 이벤트를 통해 부모 검색 상태와 동기화
- 카드 클릭 시 선택 도시 상태 표시
- 상세보기 클릭 시 날씨 알림 출력
...
- 실시간 모드에서는 입력할 때마다 `update-query` 이벤트 발화
- 지연 모드에서는 `change` 이벤트 발생 시 검색어 앞뒤 공백을 제거한 뒤 이벤트 발화
- 화면에 부모의 `searchQuery`를 표시하여 모드별 반영 시점 비교
...
1. 실시간 모드에서 `부`를 입력할 때 부산과 부천 카드가 즉시 표시되는지 확인합니다.
2. 지연 모드에서 `  부  `를 입력한 뒤 포커스를 해제했을 때 공백이 제거되고 부산과 부천 카드가 표시되는지 확인합니다.
3. 각 모드에서 화면의 부모 `searchQuery`와 콘솔 로그가 갱신되는 시점을 비교합니다.
4. 카드 클릭 시 선택 도시 상태바가 변경되는지 확인합니다.
5. 상세보기 버튼 클릭 시 알림만 출력되고 카드 선택 이벤트는 발생하지 않는지 확인합니다.
6. 검색 결과가 없을 때 결과 없음 안내가 표시되는지 확인합니다.
7. 최초 화면 로드 시 개발자 도구 콘솔에 다음 형식의 로그가 출력되는지 확인합니다.
